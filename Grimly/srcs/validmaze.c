...
/* A modified atoi to aid in validating the mapkey. 
 * Check. */
...
/* Validates the key required to read the labyrinth. If the key is invalid, it returns NULL. 
 ...
 * Edit as the map doesn't have to be square. It can be any row/column. 
 * Check. */
...
static int  valid_row(t_key *key, char *line, t_rs *s)
{
    ...
    if (s->n == 0 || s->n == key->row - 1)
        while (line[i])
        {
            if (line[i] != key->key[0] && line[i] != key->key[3] && line[i] != key->key[4])
                return (-1);
            i++;
        }
    else
        while (line[i])
        {
            if (i ==  0 || i == key->column - 1)
            {
                ...
                if (line[i] != key->key[0] && line[i] != key->key[1] && line[i] != key->key[3] && line[i] != key->key[4])
                    return (-1); 
            i++;
        }
    return (0);
...
/* Well, you learned a valuable lesson about why not to use static counter variables today. 
 * Check. */
...
/* This will check if the map is valid line by line. It'll call one function to check 
 * the first line of the given map, the map reader, and return a string of the key needed
 * to read the maze. The rest of the lines validate the rest of the labyrinth. 
 * The reason for run == key->rc + 1 is because of how GNL works. If it reads everything, it'll return 1 then read one more time and return 0. 
 * A minor change. arr and key will be together in a struct, and the struct will include row/column separately as it doesn't have to be a square.
 * Check.  */
...
/* This will validate the maze, line by line, then parse the maze into a 2D array. 
 * The struct rs is used to replace the usage of static variables. 
 * Check. You'll need to free everything you malloc soon. */
